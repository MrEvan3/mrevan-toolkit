...
A ferramenta definitiva de resgate de tela azul. Para usar, inicie o PC a partir de um PenDrive com o instalador do Windows, pressione Shift+F10 na primeira tela para abrir o CMD e execute o script.

Repara tabelas de partições e o Winload.efi com comandos como bootrec e bcdboot.

Extrai logs de falha (SrtTrail.txt) e copia arquivos bloqueados com Robocopy.

📖 Guia Passo a Passo: Quebra ou Alteração de Senha (Bypass Fantasma Multi-Vetor)
1. Prepare o PenDrive: Salve o MrEvanToolkit.ps1 na raiz de um PenDrive de instalação do Windows.

2. Dê o Boot: Inicie o PC pelo PenDrive. Quando aparecer a tela inicial de instalação do Windows, pressione Shift + F10 para abrir o CMD.

3. Abra o Script: Digite powershell e dê Enter. Digite o caminho do PenDrive onde o script está salvo (ex: D:\MrEvanToolkit.ps1). O script detectará automaticamente o modo WinPE e exibirá um Termo de Responsabilidade no terminal; digite S para aceitar.

4. Injete o Código: Acesse a opção 10, e depois a opção 5. O script usará um radar automático (Zero-Touch) para localizar em qual unidade o Windows está instalado, sem que você precise adivinhar a letra. Escolha uma das 3 portas de entrada (ex: [2] Sethc para o gatilho do Shift).

5. Mude a Senha: Reinicie o PC normalmente. Na tela de login, ative o gatilho que você escolheu (ex: aperte a tecla SHIFT 5 vezes rápidas). Um terminal com privilégios máximos (Nível Root) será aberto sobre a tela de bloqueio.

6. Redefina o Acesso: No terminal, digite control userpasswords2 para redefinir qualquer senha com interface gráfica, ou use o comando net user [nome_do_usuario] *.

7, ⚠️ Limpe os Rastros: Esta etapa é obrigatória para a segurança do cliente. Após recuperar o acesso e entrar no Windows, repita o processo de Boot pelo PenDrive. Acesse novamente a Opção 10 > 5 e escolha a Opção 4 (DESFAZER / LIMPAR RASTROS). O sistema restaurará os arquivos originais (.bak) automaticamente e fechará a vulnerabilidade.


<img width="843" height="603" alt="Captura de tela 2026-03-01 144631" src="https://github.com/user-attachments/assets/54cd67ee-513e-4e61-a848-8185c5772d28" />
